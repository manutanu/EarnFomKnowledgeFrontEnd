...
export class LoginComponent implements OnInit {
  ...
  constructor(private http:HttpClient,private router:Router,private loadingScreenService:LoadingscreenService,
    private userdetailService:UserDetailService,private snackbarservice:SnackbarService) {
  ...
  onSubmit(){
    ...
    this.signinForm.value["password"]==='' || this.signinForm.value["password"]=== undefined){
      this.loadingScreenService.stopLoading();
      this.snackbarservice.openSnackBarError("Please fill in Username and Password to Login !","close");
      return ;
    }
    ...
    obs.subscribe(data => {
            ...
            this.loadingScreenService.stopLoading();
            this.snackbarservice.openSnackBarSuccess("Welcome "+this.signinForm.value["username"]+" to KnowledgeApp !","close");
        ...
        error => {
            ...
            this.snackbarservice.openSnackBarError("Sorry it seems like you are not a Registered user !","close");
            this.loadingScreenService.stopLoading();
        }
        );
        this.loadingScreenService.stopLoading();
